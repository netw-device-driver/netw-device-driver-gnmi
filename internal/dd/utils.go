...
func startMerge(p1, p2 *config.Path, x1, x2 interface{}) (*config.Path, interface{}) {
	...
	// transform data input when object is part of a list with a key
	// this is indicated in the path where the last element in the gnmi path
	// map[string]interface{} to map[string][]interface{}
	// e.g. interfaces or subinterfaces or network-instances
	if len(p2.GetElem()) > 0 {
		if len(p2.GetElem()[len(p2.GetElem())-1].GetKey()) != 0 {
